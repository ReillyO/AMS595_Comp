...
summaryTable = table(transpose(sampleSize), transpose(piValues), transpose(deviations), transpose(times));
summaryTable
...
for sigFigs = 1:SIG_FIGS
    ...
    while ~meetsSigFigs(sigFigs, piVal)
...
%% Functions
% Check if the calculated pi value meets sig fig demands based on the
% provided number of sigfigs
...
function [valueToReturn, withinCircle, outsideCircle] = getPiValFromCoords(coordArray, currentInsideCircle, currentOutsideCircle)
    ...
    % create specified number of points and determine if they fall within
    % the inscribed circle via distance-to-center calculation
    % if a point does fall within the circle, increment the counter
